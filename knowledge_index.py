    ...
    for conv_id, data in reading_set.items():
        ...
        if "AS1" in article_data:
            ...
            for sent in sentences:
                long_sentence += sent
            text = summarize(long_sentence, ratio=0.5)
...
def build_topical_chats_knowledge_index_bert(args):
    ...
    knowledge_convo_embs = build_sent_model_embs_set_summarize_arts(reading_sets, model, knowledge_convo_embs, args.knowledge_policy)

    knowledge_index_path = os.path.join(args.data_dir, 'tc_processed', 'tc_knowledge_index_bert_test_freq_summarize.pkl')
